...
<body>
	...
	<div class="main-container">
		<div class="pd-ltr-20">
			<div class="card-box pd-20 height-100-p mb-30">
				<div class="row align-items-center">
					...
					<div class="col-md-8">
						...
						<p class="font-18 max-width-600">you play a pivotal role as the HR manager in our organization.</p>
					</div>
				</div>
			</div>
			...
			// Code for displaying upcoming birthdays and work anniversaries
			<div class="upcoming-events">
    			<h2 class="event-title">Upcoming Birthdays and Work Anniversaries</h2>
							<?php
							// Get today's date
							$currentDate = date('Y-m-d');

							// Fetch employees with upcoming work anniversaries within the next 30 days
							$sqlAnniversary = "SELECT * FROM tblemployees WHERE DATE_ADD(RegDate, INTERVAL YEAR(CURDATE())-YEAR(RegDate) YEAR) BETWEEN '$currentDate' AND DATE_ADD('$currentDate', INTERVAL 30 DAY)";
							$queryAnniversary = mysqli_query($conn, $sqlAnniversary) or die(mysqli_error());

							// Display upcoming work anniversaries
							while ($rowAnniversary = mysqli_fetch_array($queryAnniversary)) {
									$employeeNameAnniversary = $rowAnniversary['FirstName'] . " " . $rowAnniversary['LastName'];
									$anniversaryDate = $rowAnniversary['RegDate']; // Joining date
									echo "Upcoming Work Anniversary: $employeeNameAnniversary on $anniversaryDate<br>";
							}

							// Fetch employees with upcoming birthdays within the next 30 days
							$sqlBirthday = "SELECT * FROM tblemployees WHERE DATE_FORMAT(dob, '%m-%d') BETWEEN DATE_FORMAT(CURDATE(), '%m-%d') AND DATE_FORMAT(DATE_ADD(CURDATE(), INTERVAL 30 DAY), '%m-%d')";
							$queryBirthday = mysqli_query($conn, $sqlBirthday) or die(mysqli_error());

							// Display upcoming birthdays
							while ($rowBirthday = mysqli_fetch_array($queryBirthday)) {
									$employeeNameBirthday = $rowBirthday['FirstName'] . " " . $rowBirthday['LastName'];
									if (isset($rowBirthday['Dob'])) {
											$Dob = $rowBirthday['Dob'];
											echo "Upcoming Birthday: $employeeNameBirthday on $Dob<br>";
									} else {
											echo "Upcoming Birthday: $employeeNameBirthday (Date of Birth not available)<br>";
									}
							}
							?>
				</div>
